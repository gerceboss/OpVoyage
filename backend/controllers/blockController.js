...
// 2. make a function to retrive a block using blockcnumber
...
exports.latestBlock = catchAsync(async (req, res) => {
  ...
  const txIndex = block.transactions.transactionIndex;
  const options3 = {
    method: "POST",
    headers: { accept: "application/json", "content-type": "application/json" },
    body: JSON.stringify({
  ...
  const responsenew = await fetch(apiURL, options3);
  let blocknew = await resp.json();

  block.transactions = blocknew.transactions;

  const blockfinal = await Block.create({ block });
  await txController.saveTxs(blockfinal.transactions);
  res.status(200).json({
    status: "success",
    data: blockfinal,
  });
